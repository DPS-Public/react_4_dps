...
import { serviceUpdateAPICanvasShare } from "@/ui-canvas/uicApiCanvas/services/serviceUpdateAPICanvasShare";

interface APICanvasShareModalProps {
  open: boolean;
  onClose: () => void;
  canvasId: string;
  canvasTitle: string;
...
export default function APICanvasShareModal({
  open,
  onClose,
  canvasId,
  canvasTitle,
  ...
  const shareUrl = shareToken
    ? `${window.location.origin}/api-canvas/share/${shareToken}`
    : "";

  const handleShareToggle = async (checked: boolean) => {
    setLoading(true);
    try {
      const result = await serviceUpdateAPICanvasShare(canvasId, checked, shareToken);
      setIsShared(checked);
      setShareToken(result.shareToken || "");
      message.success(checked ? "API Canvas shared successfully" : "API Canvas unshared successfully");
    } catch (error) {
  ...
  const handleRegenerateShareId = async () => {
    setLoading(true);
    try {
      const result = await serviceUpdateAPICanvasShare(canvasId, isShared, shareToken, true);
      setShareToken(result.shareToken || "");
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 12, color: "#999", marginTop: 12 }}>
                <p style={{ margin: "8px 0" }}>
                  ✓ Anyone with this link can view this API canvas (read-only)
                </p>
                <p style={{ margin: "8px 0" }}>
          ...
          {!isShared && (
            <div style={{
              ...
              Enable public sharing to generate a shareable link for this API canvas.
            </div>
          )}
        </div>
      </Spin>
