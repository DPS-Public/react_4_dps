...
import Commit, { CommitFile } from '../types/Commit.interface';
 ...
 */
const AllFilesModal: React.FC<AllFilesModalProps> = ({ visible, commit, onClose, onViewFile }) => (
    <Modal
        title={
            <div>
                ...
                {commit && (
                    <div style={{ marginTop: 4, fontSize: 13, fontWeight: 'normal', color: '#666' }}>
                        {commit.sha.substring(0, 7)} – {commit.commit?.message || commit.message || 'No message'}
                    </div>
                )}
            </div>
        }
                        ...
                        <List.Item
                            ...
                            actions={[
                                <Button
                                    ...
                                    onClick={() => { onClose(); onViewFile(commit, file); }}
                                >
                                    View Changes
                                </Button>,
                            ]}
